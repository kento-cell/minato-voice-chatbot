...
**マイクボタンを押しながら話すと、キャラが考えて声で返します**（テキスト入力も可）。
会話は直近10ターンを記憶（セッション内のみ・保存なし）。キャラの切り替え・履歴リセットもUIから。
...
- ~~マイク入力は未実装~~ → **Phase 2完了**: ブラウザ音声チャットで対応済み（`src/webapp.py`）
